...
function s.select(e,tp,b1,b2)
	...
	if op==1 then
		--Equip 1 DARK Warrior to your "Phantom Gunners"
...
function s.eqfilter1(c)
	return c:IsFaceup() and c:IsSetCard(0x302) and c:IsType(TYPE_EFFECT) 
		and Duel.IsExistingMatchingCard(s.eqfilter2,0,LOCATION_GRAVE,0,1,nil)
end
...
function s.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	if ec and ec:IsRelateToEffect(e) and ec:IsFaceup() then
		...
		local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.eqfilter2),tp,LOCATION_GRAVE,0,1,1,nil)
		local tc=g:GetFirst()
...
function s.descfilter(c)
	...
	return ec and ec:IsSetCard(0x302) and c:IsAbleToGraveAsCost()
end
