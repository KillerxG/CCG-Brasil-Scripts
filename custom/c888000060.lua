...
function s.spfilter(c,e,tp,sync)
	return (c:IsCanBeSpecialSummoned(e,0,tp,false,false) or c:IsAbleToRemove()) and (c:IsFaceup() or not c:IsLocation(LOCATION_EXTRA))
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_GRAVE+LOCATION_EXTRA,0,1,nil,e,tp,sync) end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.spfilter,tp,LOCATION_GRAVE+LOCATION_EXTRA,0,1,1,nil,e,tp,sync)
	if #g>0 then
	    local tc=g:GetFirst()
		if tc and tc:IsCanBeSpecialSummoned(e,0,tp,false,false) and (not tc:IsAbleToRemove() or Duel.SelectYesNo(tp,aux.Stringid(id,1))) then
