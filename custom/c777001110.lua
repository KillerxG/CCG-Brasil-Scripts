...
function s.aecostfilter(c)
  return c:GetCounter(0x1297)>0
end
function s.aecost(e,tp,eg,ep,ev,re,r,rp,chk)
  if chk==0 then return Duel.GetCounter(tp,1,0,0x1297)>0 end
  local g=Duel.GetMatchingGroup(s.aecostfilter,tp,LOCATION_ONFIELD,0,nil)
  local ct=0
  for tc in aux.Next(g) do
    local sct=tc:GetCounter(0x1297)
    tc:RemoveCounter(tp,0x1297,sct,0)
    ct=ct+sct
  end
  e:SetLabel(ct)
end
...
function s.aeop(e,tp,eg,ep,ev,re,r,rp)
  ...
  if ct>=3 then
    ...
    e2:SetTarget(aux.TargetBoolFunction(Card.IsSetCard,0x297))
    e2:SetValue(1)
...
function s.actfilter(c,tp)
  return c:IsFaceup() and c:IsSetCard(0x297) and c:IsControler(tp)
end
function s.actcon(e)
